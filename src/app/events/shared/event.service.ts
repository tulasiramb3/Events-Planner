...
export class EventService {
  constructor(private http:HttpClient) {
    ...
    //   EVENTS[index]= event
    // }
    getSessions(sessionName:string) {
      return this.http.get<ISession[]>('api/sessions/search?search='+sessionName)
      .pipe(catchError(this.handleError<ISession[]>('searchSessions')));
      // let results : ISession[] = []
      // console.log(sessionName);
      //  EVENTS.forEach(event=>{
      //   var filterSession= event.sessions.filter(session=>
      //         session.name.toLocaleLowerCase().indexOf(sessionName.toLocaleLowerCase())>-1);
      //     filterSession = filterSession.map((session:any)=>{
      //           session.eventId=event.id
      //           return session;
      //     })
      //    results = results.concat(filterSession);
      //   })
...
//     //     country: 'Netherlands'
//     //   },
//       onlineUrl:"here goes online URL",
//       sessions: [
//         {
//           id: 1,
